...
export function buildSafeHtmlDocument(
  ...
  const htmlInteractionBridge = allowScripts
    ? `<script>(function(){var request=function(url){if(typeof url==='string'&&(url.toLowerCase().indexOf('http://')===0||url.toLowerCase().indexOf('https://')===0))parent.postMessage({type:'lightpage-external-link',url:url},'*');return null;};window.open=${allowPopups ? 'request' : 'function(){return null;}'};document.addEventListener('click',function(event){var link=event.target&&event.target.closest?event.target.closest('a[data-external-href]'):null;if(!link)return;event.preventDefault();request(link.getAttribute('data-external-href'));},true);window.addEventListener('message',function(event){var data=event.data;if(event.source!==parent||!data||data.type!=='lightpage-scroll-to-heading'||typeof data.id!=='string'||data.id.length>1024)return;var target=document.getElementById(data.id);if(target&&typeof target.scrollIntoView==='function')target.scrollIntoView({block:'start'});});})();</script>`
    : ''
  const formActionPolicy = allowForms ? "'self'" : "'none'"
  const contentSecurityPolicy = allowExternalResources
    ? `default-src 'self' data: blob: http: https:; script-src 'unsafe-inline' http: https:; style-src 'unsafe-inline' http: https:; connect-src http: https:; object-src 'none'; frame-src 'none'; form-action ${formActionPolicy}; navigate-to 'none'; base-uri about:`
    : `default-src 'none'; script-src 'unsafe-inline'; img-src data: blob: http://localhost/_capacitor_file_ capacitor://localhost; style-src 'unsafe-inline' http://localhost/_capacitor_file_ capacitor://localhost; font-src data: http://localhost/_capacitor_file_ capacitor://localhost; connect-src 'none'; object-src 'none'; frame-src 'none'; form-action ${formActionPolicy}; navigate-to 'none'; base-uri about:`

  return {
    srcDoc: `<!doctype html><html><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><meta http-equiv="Content-Security-Policy" content="${contentSecurityPolicy}" /><base href="about:srcdoc" target="_self" /><style>html{-webkit-text-size-adjust:100%}body{overflow-wrap:anywhere}img,video,canvas,svg{max-width:100%;height:auto}.lp-missing-resource{display:block;margin:8px 0 14px;padding:9px 10px;border-radius:8px;color:#9a5b08;background:#fff1d8;font-size:.86em}mark.search-hit{padding:0 2px;border-radius:3px;color:#1f1600;background:#ffd86b}</style><title>${escapeHtml(title)}</title>${htmlInteractionBridge}${headHtml}</head><body>${bodyHtml}</body></html>`,
    headings,
    plainText,
    externalResources,
  }
