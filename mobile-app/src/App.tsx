...
function App() {
  ...
  const desktopCommands: DesktopCommand[] = [
    ...
    { id: 'document.toc', title: '显示或隐藏章节目录', shortcut: 'Ctrl+Shift+B', enabled: () => view === 'reader', run: () => dispatchReaderCommand('toc') },
    { id: 'document.bookmark', title: '添加章节书签', shortcut: 'Ctrl+B', enabled: () => view === 'reader' && activeDocument?.fileType === 'markdown', run: () => dispatchReaderCommand('bookmark') },
    { id: 'document.highlight', title: '高亮所选文字', shortcut: 'Ctrl+Shift+H', enabled: () => view === 'reader' && activeDocument?.fileType === 'markdown', run: () => dispatchReaderCommand('highlight') },
