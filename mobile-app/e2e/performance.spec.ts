...
test('1 MB 文档打开与搜索保持在发布性能门槛内', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '一兆文档', exact: true }).first()).toBeVisible()
  expect(Date.now() - openStartedAt).toBeLessThan(2500)
  ...
  expect(Date.now() - searchStartedAt).toBeLessThan(1500)
