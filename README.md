...
- 支持复制 Markdown 源文和带标题、列表、表格、代码、链接及图片的富文本，以及图片、分页长图、PDF 和原始文件导出；Android 还可导出批注摘要。
...
- 监听当前文档及其相对资源的外部修改，通过原生事件与文件时间戳双重检测自动刷新；刷新只采用稳定文件快照，并在内容变化时重建 Markdown 正文，避免保存过程中出现瞬时残缺或渲染冲突。文件菜单的“重新加载”及 `Ctrl+R` 可随时手动同步。
- 使用 `Ctrl+Shift+P` 打开命令面板，可执行打开、重新加载、搜索、目录、收藏、主题、字号、导出和关闭等操作。
- 自动保存窗口位置、大小和最大化状态。
...
- 大型 Markdown 会在复制前临时完成全部分块渲染，避免遗漏尚未进入视口的正文。
- Android 使用原生 HTML 剪贴板，Windows 使用 WebView2 富文本剪贴板并提供兼容兜底。
- Windows 当前文件在外部保存后会自动同步，原生文件事件失效时由时间戳轮询兜底；刷新会等待文件稳定并自动恢复偶发的瞬时渲染冲突，也可通过文件菜单“重新加载”或 `Ctrl+R` 手动刷新。
